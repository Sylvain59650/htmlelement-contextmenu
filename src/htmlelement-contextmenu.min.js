...
class ContextMenu {
  ...
  moveSelected(direction) {
    ...
    if (contextmenu.style.display === "block") {
      ...
      this.__addClasses(menuItems[contextmenu.selectedIndex], classNames.hover);
    }
  }
  ...
  redrawItems() {
    ...
    if (this.__menuInfo.items) {
      for (let i = 0; i < this.__menuInfo.items.length; i++) {
        this.makeMenuItem(this.__menuInfo.items[i], i, this.contextmenu);
      }
    }
  }
  ...
  makeMenuItem(option, nth, parent) {
    var li = document.createElement("li");
    li.classList.add(classNames.menuItem);
    if (option.label === "sep") {
      this.__addClasses(li, classNames.separator + " " + classNames.notSelectable);
    } else {
      if (option.html) {
      ...
      this.__addClasses(option.class);
      // if (option.icon) {
      //   debugger;
